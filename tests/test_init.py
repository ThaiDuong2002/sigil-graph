...
def test_register_mcp_claude_creates_file(tmp_path):
    ...
    assert "sigil" in data["mcpServers"]
    assert data["mcpServers"]["sigil"]["command"] == "sigil-mcp"
    assert str(tmp_path) in data["mcpServers"]["sigil"]["args"]
